...
class CardService {
  void showImageDialog(BuildContext context, DigimonCard card, Function(int)? searchNote) {
    final isPortrait = MediaQuery.of(context).orientation == Orientation.portrait;
    final screenWidth = MediaQuery.of(context).size.width;
    final fontSize =  min(screenWidth * 0.03,15.0); // 화면 너비의 4%로 폰트 크기 설정
                ...
                children: [
                  Container(
                    ...
                    child: Wrap(
                      ...
                      children: [
                        Text(
                          '${card.cardNo}',
                          style: TextStyle(fontSize: fontSize, color: Theme.of(context).hintColor),
                        ),
                        Text(
                          '${card.rarity}',
                        ...
                        Text(
                          '${card.getKorCardType()}',
                          style: TextStyle(fontSize: fontSize, color: ColorService.getColorFromString(card.color1!)),
                        ),
                        if (card.lv != null)
                          ElevatedButton(
                            onPressed: null,
                  ...
                  Row(
                    ...
                    children: [
                      Text(
                        '${card.cardName}',
                        style: TextStyle(fontSize: fontSize * 1.2), // 카드 이름은 폰트 크기를 1.2배로 설정
                      ),
                    ],
                  ),
                  SizedBox(height: 10),
                    ...
                    children: [
                      Expanded(
                        ...
                        child: Column(
                          ...
                          children: [
                            Stack(
                              children: [
                                Image.network(card.imgUrl ?? '', fit: BoxFit.fill),
                                Positioned(
                                  right: 0,
                                  bottom: 0,
                                  child: IconButton(
                                    ...
                                    icon: Icon(
                                      ...
                                      size: fontSize * 1.5, // 아이콘 크기도 폰트 크기에 비례하게 조정
                                    ),
                                  ),
                                ),
                              ],
                      ...
                      Expanded(
                        ...
                        child: Column(
                          children: [
                            if (card.form != null)
                              _attributeWidget(context, [card.getKorForm()], '형태',
                                  ColorService.getColorFromString(card.color1!), fontSize),
                            if (card.attributes != null)
                              _attributeWidget(context, [card.attributes!], '속성',
                                  ColorService.getColorFromString(card.color1!), fontSize),
                ...
                child: Wrap(
                    ...
                    children: attributes.map((attribute) =>
                        Chip(
                          label: Text(
                            attribute,
                            style: TextStyle(fontSize: fontSize * 0.9), // Chip 내부 텍스트의 폰트 크기를 조정
                          ),
                        )).toList()
                )
            ),
